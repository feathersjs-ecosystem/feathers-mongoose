...
class Service extends AdapterService {
  constructor (options) {
    ...
    this.maxTotalLimit = options.maxTotalLimit === undefined ? 0 : options.maxTotalLimit;
  }
  ...
  _find (params = {}) {
    ...
    if (paginate && paginate.default) {
      let q = model.where(query)[this.useEstimatedDocumentCount ? 'estimatedDocumentCount' : 'countDocuments']();
      if (this.maxTotalLimit > 0) q.limit(this.maxTotalLimit);
      return q.session(params.mongoose && params.mongoose.session).exec().then(executeQuery);
    }

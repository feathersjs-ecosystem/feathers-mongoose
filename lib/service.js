...
class Service {
  ...
  _get (id, params = {}) {
    ...
    // Add all properties without a $-prefix to the query object used for retrieving the record.
    const findOneQuery = {
        [this.id] : id,
    };
    for (const propertyName in params.query) {
        if (propertyName[0] !== '$') {
            findOneQuery[propertyName] = params.query[propertyName];
        }
    }
    ...
    return modelQuery
      ...
      .then(data => {
        if (!data) {
          throw new errors.NotFound(`No record found for the given query targeting id '${id}'`);
        }
